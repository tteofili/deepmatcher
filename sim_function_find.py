...
from csv2dataset import csv_2_datasetALTERNATE, csvTable2datasetRANDOM_likeGold, read_data_bf
...
def find_sim(gt_file, t1_file, t2_file, indexes, simfunctions, soglia, tot_pt, tot_copy):
    best = []
    for r in range(3):
...
tot_pt = 100  # dimensione dataset pre_training
tot_copy = 40 # numero di elementi generati con edit distance
soglia = 0.03  # da aggiungere per discostarsi da min_sim e max_sim ottenuto
get_lambda_name = lambda l: getsource(l).split('=')[0].strip()


print(f'finding best step fit function for {DATASET_NAME}')
print(find_sim(GROUND_TRUTH_FILE, TABLE1_FILE, TABLE2_FILE, ATT_INDEXES, simfunctions))'''